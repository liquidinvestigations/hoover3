...
pub async fn get_mime_type(path: PathBuf) -> anyhow::Result<String> {
    let mime_type = tokio::task::spawn_blocking(move || file_example(path)).await??;
    Ok(mime_type)
}

fn file_example(path: PathBuf) -> Result<String, anyhow::Error> {
