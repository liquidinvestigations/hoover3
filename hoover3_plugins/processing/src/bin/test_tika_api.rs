...
async fn main() -> anyhow::Result<()> {
    ...
    for file in files {
        ...
        println!(
            "{}: filesize = {}k, after {} ms, resp len: {}k",
...
async fn run_tika_api(path: &std::path::Path) -> anyhow::Result<String> {
    let mime =
        hoover3_processing::tasks::get_mime_type::magic_get_mime_type(path.to_path_buf()).await?;
    ...
    let response = client
        ...
        .header(
            "Content-Type",
            mime.magic_mime_type.split(";").into_iter().next().unwrap(),
        )
        .send()
        .await?;
