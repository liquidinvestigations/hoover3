...
use quote::quote;
use syn::ReturnType;
use syn::{FnArg, ItemFn};use syn::Type;use syn::PathArguments;
...
fn extract_activity_arg_type(f: &ItemFn) -> Box<syn::Type> {
    ...
    assert!(args_list.len() == 1, "#[activity] must have exactly one argument");
    let arg = args_list[0];
...
fn extract_activity_return_type(f: &ItemFn) -> Box<syn::Type> {
    ...
    let Type::Path(ret) = *ret else {panic!("invalid return type")};
    let last_segment = ret.path.segments.last().expect("return type has segment");
    assert!(last_segment.ident == "Result", "not a result");
    let PathArguments::AngleBracketed(args) = last_segment.arguments.clone() else {panic!("invalid return type")};
    assert!(args.args.len() == 1, "invalid activity return type: plz use anyhow::Result<T>");
    let GenericArgument::Type(ret) = args.args[0].clone() else {panic!("invalid return type")};
    Box::new(ret)
}
...
fn extract_workflow_arg_type(f: &ItemFn) -> Box<syn::Type> {
    let args = f.sig.inputs.clone();
    let args_list = args.iter().collect::<Vec<_>>();
    assert!(args_list.len() == 2, "#[workflow] must have exactly two arguments (ctx and input)");
    let arg = args_list[1];
    let FnArg::Typed(arg_pattern) = arg else {
...
fn extract_workflow_return_type(f: &ItemFn) -> Box<syn::Type> {
    ...
    let Type::Path(ret) = *ret else {panic!("invalid return type")};
    let last_segment = ret.path.segments.last().expect("workflow return type has segment");
    assert!(last_segment.ident == "WorkflowResult", "not a WorkflowResult");
    let PathArguments::AngleBracketed(args) = last_segment.arguments.clone() else {panic!("invalid return type")};
    assert!(args.args.len() == 1, "invalid workflow return type: plz use WorkflowResult<T>");
    let GenericArgument::Type(ret) = args.args[0].clone() else {panic!("invalid return type")};
    Box::new(ret)
}
